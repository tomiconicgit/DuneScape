...
// Import CapsuleGeometry for the new limb shapes
...
export default class Player {
    ...
    createCharacterRig() {
        ...
        // Use Capsules for limbs for a more organic look
        const createLimb = (length, isArm) => {
            const group = new THREE.Group();
            const radius = isArm ? 0.15 : 0.18;
            
            const upperGeo = new CapsuleGeometry(radius, length, 4, 16);
            const upper = new THREE.Mesh(upperGeo, bodyMaterial);
            upper.castShadow = true;
            upper.position.y = -length / 2;
            
            const lowerGeo = new CapsuleGeometry(radius * 0.9, length * 0.9, 4, 16);
