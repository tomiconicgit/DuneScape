...
const DAY_DURATION_SECONDS = 120; const NIGHT_DURATION_SECONDS = 120;
const TOTAL_CYCLE_SECONDS = DAY_DURATION_SECONDS + NIGHT_DURATION_SECONDS;
const SUN_COLOR_NOON = new THREE.Color().setHSL(0.1, 1, 0.95); const SUN_COLOR_SUNSET = new THREE.Color().setHSL(0.05, 1, 0.7);
const HEMI_SKY_COLOR_NOON = new THREE.Color().setHSL(0.6, 1, 0.6); const HEMI_GROUND_COLOR_NOON = new THREE.Color().setHSL(0.095, 1, 0.75);
const HEMI_COLOR_SUNSET = new THREE.Color().setHSL(0.05, 0.5, 0.7); const HEMI_SKY_COLOR_NIGHT = new THREE.Color().setHSL(0.6, 0.1, 0.05);
...
export default class Game {
    ...
    handleNavigation(location) {
        ...
        this.movement.followTrail(trailPoints, this.terrain.mesh);
    }
    ...
    _setupPostProcessing() {
        ...
        this.bloomPass.threshold = 0.2; this.bloomPass.strength = 0.0; this.bloomPass.radius = 0.5;
        this.composer.addPass(this.bloomPass);
    }

    _setupEvents() {
        window.addEventListener('resize', () => {
            this.camera.handleResize(); this.renderer.setSize(window.innerWidth, window.innerHeight);
    ...
    _updateMirageEffect() {
        ...
        const quadrantSize = 400; const halfQuadrant = quadrantSize / 2;
        const gridX = Math.abs((playerPos.x % quadrantSize + quadrantSize) % quadrantSize - halfQuadrant);
        const gridZ = Math.abs((playerPos.z % quadrantSize + quadrantSize) % quadrantSize - halfQuadrant);
        const distToEdge = Math.min(halfQuadrant - gridX, halfQuadrant - gridZ);
        const mirageStrength = 1.0 - smoothstep(0, halfQuadrant, distToEdge);
        this.bloomPass.strength = mirageStrength * 0.4;
    }
    ...
    _animate() {
        ...
        if(cycleProgress<(DAY_DURATION_SECONDS/TOTAL_CYCLE_SECONDS)){dayProgress=cycleProgress/(DAY_DURATION_SECONDS/TOTAL_CYCLE_SECONDS);currentElevation=Math.sin(dayProgress*Math.PI)*90;}else{const nightProgress=(cycleProgress-(DAY_DURATION_SECONDS/TOTAL_CYCLE_SECONDS))/(NIGHT_DURATION_SECONDS/TOTAL_CYCLE_SECONDS);currentElevation=Math.sin(Math.PI+nightProgress*Math.PI)*90;dayProgress=1.;}
        currentAzimuth=180-(dayProgress*360);
        this.sky.setParameters({elevation:currentElevation,azimuth:currentAzimuth});this.sunLight.position.copy(this.sky.sun).multiplyScalar(1000);
        
        // --- Lighting Update ---
        const sunHeightFactor=Math.max(0,currentElevation)/90;const sunsetFactor=smoothstep(.4,0.,sunHeightFactor);const nightFactor=smoothstep(0.,-.2,currentElevation/90.);
        this.sunLight.intensity=(1.-nightFactor)*3.;this.sunLight.color.lerpColors(SUN_COLOR_NOON,SUN_COLOR_SUNSET,sunsetFactor);
        this.hemiLight.intensity=(.2+sunHeightFactor*2.)*(1.-nightFactor*.5);const daySkyHemi=HEMI_SKY_COLOR_NOON.clone().lerp(HEMI_COLOR_SUNSET,sunsetFactor);const dayGroundHemi=HEMI_GROUND_COLOR_NOON.clone().lerp(HEMI_COLOR_SUNSET,sunsetFactor);this.hemiLight.color.lerpColors(daySkyHemi,HEMI_SKY_COLOR_NIGHT,nightFactor);this.hemiLight.groundColor.lerpColors(dayGroundHemi,HEMI_GROUND_COLOR_NIGHT,nightFactor);
        
        // --- Rock Shader Update ---
        this.scene.traverse(child=>{if(child.isMesh&&child.material.uniforms){if(child.material.uniforms.lightDir){child.material.uniforms.lightDir.value.copy(this.sunLight.position);}
        if(child.material.uniforms.sunColor){const sun=this.sunLight.color.clone().multiplyScalar(this.sunLight.intensity);child.material.uniforms.sunColor.value.copy(sun);}
        if(child.material.uniforms.hemiSkyColor){const hemiSky=this.hemiLight.color.clone().multiplyScalar(this.hemiLight.intensity);child.material.uniforms.hemiSkyColor.value.copy(hemiSky);}
        if(child.material.uniforms.hemiGroundColor){const hemiGround=this.hemiLight.groundColor.clone().multiplyScalar(this.hemiLight.intensity);child.material.uniforms.hemiGroundColor.value.copy(hemiGround);}}});

        if (this.scene.fog) { this.scene.fog.color.copy(this.hemiLight.groundColor); }
