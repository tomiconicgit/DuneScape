...
export default class Game {
  ...
  async init() {
    ...
    window.loader.updateStatus('Loading player model...', 75);
    this.player = new Player(this.scene, this);
    await this.player.loadModel(); // Wait for the model to load
  ...
  // (The rest of Game.js remains the same, so it is omitted for brevity)
  ...
  setupInitialScene() {
    ...
    window.loader.updateStatus('Building pathfinding grid...', 70);
  ...
  procedurallyPlaceRocks() {
    ...
    for (const [rockType, properties] of Object.entries(zones)) {
      ...
      for (let i = 0; i < properties.count; i++) {
        ...
        placedCount++;
        const progress = 40 + (placedCount / totalRocks) * 30;
