...
export default class Movement {
    ...
    calculatePathOnTerrain(targetWorldPos, terrainMesh) {
        ...
        const path3D = flatPath.map(p => new THREE.Vector3(p.x, 0, p.z));
        this.followTrail(path3D, terrainMesh);
        this._placeMarker(targetWorldPos.x, targetWorldPos.y + 0.1, targetWorldPos.z);
    }
    ...
    _placeMarker(x, y, z) {
        ...
        if (!this.marker.parent) this.character.parent.add(this.marker);
    }
    ...
    _findPath(start, goal) {
        ...
        for (let i = 0; i <= segments; i++) {
            const t = i / segments;
            path.push({ x: (1 - t) * start.x + t * goal.x, z: (1 - t) * start.z + t * goal.z });
        }
        return path;
    }
}
