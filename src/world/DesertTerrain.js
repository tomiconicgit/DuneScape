...
export class DesertTerrain {
    ...
    constructor(scene, townDimensions) {
        ...
        this.config = {
            ...
            distanceBlur: { enabled: true, startDistance: desertSize * 0.35, endDistance: desertSize * 0.5, skyboxColor: new THREE.Color(0xaad6f5), atmosphericHaze: true, hazeStartDistance: desertSize * 0.15, hazeFactor: 0.6 },
    ...
    getDirectionalDuneHeight(x, z) {
        ...
        const rotX = x * Math.cos(direction) + z * Math.sin(direction); const rotZ = -x * Math.sin(direction) + z * Math.cos(direction);
        const duneHeight = this.duneNoise.noise(rotX * this.config.noiseScale.dunes, rotZ * this.config.noiseScale.dunes * 0.5) * this.config.heightScale.dunes;
        const secondaryHeight = this.secondaryDuneNoise.noise(rotX * this.config.noiseScale.secondaryDunes, rotZ * this.config.noiseScale.secondaryDunes) * this.config.heightScale.secondaryDunes;
        ...
        } else { ridgeHeight = (Math.abs(ridges * 2 - 1)) * this.config.heightScale.ridges; }
        return duneHeight + secondaryHeight + ridgeHeight;
    }
    isNearTrainTrack(x, z) {
        const trackStart = new THREE.Vector3(0, 0, -1000); const trackEnd = new THREE.Vector3(0, 0, 1000);
    ...
    getTownBlendFactor(x, z) {
        ...
        if (this.isNearTrainTrack(x, z)) return 0;
        if (distFromTown < this.config.townBuffer) { return 0; }
        else if (distFromTown < this.config.townBuffer * 1.5) { const transitionFactor = (distFromTown - this.config.townBuffer) / (this.config.townBuffer * 0.5); return Math.pow(transitionFactor, 2.0); }
        else { return 1.0; }
    }
    generateTerrain() {
        ...
        const heightAtEdge = this.config.size * 0.03;
        for (let i = 0; i < vertices.length; i += 3) {
            ...
            if (this.config.dunes.smoothing) { const smoothingNoise = this.baseNoise.noise(x * this.config.noiseScale.dunes * 2, z * this.config.noiseScale.dunes * 2); duneHeight *= (0.85 + smoothingNoise * 0.3); }
            ...
            if (distFromCenter > edgeFadeStart) { const edgeFactor = 1.0 - Math.min(1, (distFromCenter - edgeFadeStart) / (edgeFadeEnd - edgeFadeStart)); height *= edgeFactor; }
            vertices[i + 1] = height;
            ...
            if (this.config.distanceBlur.atmosphericHaze && distFromCenter > this.config.distanceBlur.hazeStartDistance) {
                const distanceFactor = Math.min(1.0, (distFromCenter - this.config.distanceBlur.hazeStartDistance) / (this.config.distanceBlur.endDistance - this.config.distanceBlur.hazeStartDistance));
            ...
            if (this.config.distanceBlur.enabled && distFromCenter > this.config.distanceBlur.startDistance) {
                const blurFactor = Math.min(1.0, (distFromCenter - this.config.distanceBlur.startDistance) / (this.config.distanceBlur.endDistance - this.config.distanceBlur.startDistance));
                ...
                if (distFromCenter > this.config.distanceBlur.startDistance) { const heightBlendFactor = Math.pow(blurFactor, 2.0); vertices[i + 1] += heightAtEdge * heightBlendFactor; }
            }
        }
        geometry.setAttribute('color', new THREE.BufferAttribute(colors, 3));
        geometry.computeVertexNormals();
        const sandMaterial = new THREE.MeshPhongMaterial({ vertexColors: true, shininess: 0, specular: new THREE.Color(0x000000), normalMap: normalMapTexture, normalScale: new THREE.Vector2(1.2, 1.2), fog: true });
        ...
        this.terrainMesh.position.y = -0.05; this.terrainMesh.position.set(0, -0.05, 0);
        this.scene.add(this.terrainMesh);
        return this.terrainMesh;
    }
    createSandNormalMap() {
        const size = 1024; const data = new Uint8Array(size * size * 4); const normalStrength = 40;
        for (let y = 0; y < size; y++) {
            for (let x = 0; x < size; x++) {
                const nx = x / size, ny = y / size;
                const windDirection = this.config.duneDirection;
                const windAlignedX = nx * Math.cos(windDirection) + ny * Math.sin(windDirection), windAlignedY = -nx * Math.sin(windDirection) + ny * Math.cos(windDirection);
                const ripples = this.microRipplesNoise.noise(windAlignedX * 35, windAlignedY * 12) * 1.8;
                const grains = this.sandGrainsNoise.noise(nx * 200, ny * 200) * 0.2;
                const mediumVar = this.detailNoise.noise(nx * 40, ny * 40) * 0.5;
                const combined = ripples * 0.8 + grains * 0.3 + mediumVar * 0.2;
                const idx = (y * size + x) * 4;
                const left = x > 0 ? ripples * 0.8 + this.sandGrainsNoise.noise((nx - 1/size) * 200, ny * 200) * 0.2 + this.detailNoise.noise((nx - 1/size) * 40, ny * 40) * 0.3 : combined;
                const right = x < size-1 ? ripples * 0.8 + this.sandGrainsNoise.noise((nx + 1/size) * 200, ny * 200) * 0.2 + this.detailNoise.noise((nx + 1/size) * 40, ny * 40) * 0.3 : combined;
                const up = y > 0 ? ripples * 0.8 + this.sandGrainsNoise.noise(nx * 200, (ny - 1/size) * 200) * 0.2 + this.detailNoise.noise(nx * 40, (ny - 1/size) * 40) * 0.3 : combined;
                const down = y < size-1 ? ripples * 0.8 + this.sandGrainsNoise.noise(nx * 200, (ny + 1/size) * 200) * 0.2 + this.detailNoise.noise(nx * 40, (ny + 1/size) * 40) * 0.3 : combined;
        ...
        texture.repeat.set(15, 15); texture.needsUpdate = true;
        return texture;
    }
    createSandRoughnessMap() {
        const size = 512; const data = new Uint8Array(size * size);
        for (let y = 0; y < size; y++) {
            for (let x = 0; x < size; x++) {
                const nx = x / size, ny = y / size;
                const baseRoughness = 245;
                const windDirection = this.config.duneDirection;
                const alignedX = nx * Math.cos(windDirection) + ny * Math.sin(windDirection), alignedY = -nx * Math.sin(windDirection) + ny * Math.cos(windDirection);
                ...
                const roughness = Math.min(255, Math.max(230, baseRoughness + (windPattern < 0 ? 10 : -5) + fineGrains));
                data[y * size + x] = roughness;
            }
        }
        const texture = new THREE.DataTexture(data, size, size, THREE.RedFormat);
        texture.wrapS = THREE.RepeatWrapping; texture.wrapT = THREE.RepeatWrapping;
        texture.repeat.set(20, 20); texture.needsUpdate = true;
        return texture;
    }
    addCacti() {
        const cactusMaterial = new THREE.MeshStandardMaterial({ color: 0x2d5c2d, roughness: 0.8, metalness: 0.2, });
        const segmentGeometries = [this.createCactusTrunk(2.5, 4, 20, 8), this.createCactusArm(1.8, 2.2, 8, 8), this.createCactusTrunk(1.5, 1.8, 12, 8),];
        const cactusCount = this.config.cactiCount;
        const instancedSegments = [];
        for (let i = 0; i < segmentGeometries.length; i++) {
            const instancedMesh = new THREE.InstancedMesh(segmentGeometries[i], cactusMaterial, cactusCount * (i === 0 ? 1 : 2));
        ...
        for (let i = 0; i < cactusCount; i++) {
            ...
            const minRadius = Math.max(this.townDimensions.width, this.townDimensions.length) + 150;
            const maxRadius = this.config.size * 0.3;
            const radius = minRadius + Math.random() * (maxRadius - minRadius);
            x = Math.cos(angle) * radius; z = Math.sin(angle) * radius;
            townBlend = this.getTownBlendFactor(x, z);
            if (townBlend < 0.9) { const adjustedRadius = minRadius * 1.2; x = Math.cos(angle) * adjustedRadius; z = Math.sin(angle) * adjustedRadius; townBlend = this.getTownBlendFactor(x, z); }
            ...
            const cactusScale = 0.3 + Math.random() * 0.5; const trunkRotation = Math.random() * Math.PI * 2;
            position.set(x, y, z); rotation.set(0, trunkRotation, 0); quaternion.setFromEuler(rotation); scale.set(cactusScale, cactusScale, cactusScale);
            matrix.compose(position, quaternion, scale); instancedSegments[0].setMatrixAt(i, matrix);
            for (let arm = 0; arm < 2; arm++) {
                const armHeight = 5 + Math.random() * 8, armAngle = arm * Math.PI + (Math.random() * 0.5 - 0.25), armLength = 4 + Math.random() * 3;
                const armX = x + Math.cos(trunkRotation + armAngle) * 2 * cactusScale, armZ = z + Math.sin(trunkRotation + armAngle) * 2 * cactusScale, armY = y + armHeight * cactusScale;
                position.set(armX, armY, armZ); rotation.set(Math.PI/2, trunkRotation + armAngle + Math.PI/2, 0); quaternion.setFromEuler(rotation);
                matrix.compose(position, quaternion, scale); instancedSegments[1].setMatrixAt(i * 2 + arm, matrix);
                const tipX = armX + Math.cos(trunkRotation + armAngle) * armLength * cactusScale, tipZ = armZ + Math.sin(trunkRotation + armAngle) * armLength * cactusScale, tipY = armY;
                position.set(tipX, tipY, tipZ); rotation.set(0, trunkRotation, 0); quaternion.setFromEuler(rotation);
                matrix.compose(position, quaternion, scale); instancedSegments[2].setMatrixAt(i * 2 + arm, matrix);
            }
        }
        for (const instancedMesh of instancedSegments) { instancedMesh.instanceMatrix.needsUpdate = true; this.scene.add(instancedMesh); }
    ...
    generate() { console.log("Generating procedural desert terrain..."); this.generateTerrain(); this.addCacti(); console.log("Desert terrain generation complete"); }
}
