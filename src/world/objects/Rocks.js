...
// Master seed for the pseudo-random number generator to ensure determinism
const MASTER_SEED = 42; 

export default class Rocks {
    constructor(scene) {
        // Create a seeded pseudo-random number generator for all decisions
        this.prng = seededRandom(MASTER_SEED);

        // --- Ore Presets ---
        this.orePresets = {
            limestone: { color: 0xbfbfa8, color2: 0x9e9b84, roughness: 0.9, metalness: 0.0, displacement: 0.2, seed: 10 },
            carbon:    { color: 0x4a4a4a, color2: 0x1f1f1f, roughness: 0.6, metalness: 0.1, displacement: 0.3, seed: 20 },
            iron:      { color: 0x7d584a, color2: 0x5c3e33, roughness: 0.5, metalness: 0.4, displacement: 0.4, seed: 30 },
            stone:     { color: 0x808080, color2: 0x404040, roughness: 0.8, metalness: 0.2, displacement: 0.25, seed: null } // Null seed means it will be randomized
        };

        // --- Place Rocks on Mine Levels ---
        MINE_AREA.levels.forEach(level => {
            const rockCount = Math.floor(level.radius * 4);
            for (let i = 0; i < rockCount; i++) {
                const angle = this.prng() * Math.PI * 2;
                const radius = level.radius - (this.prng() * level.radius * 0.7);
                ...
                // 30% chance for a rock to be generic stone, otherwise it's the level's ore
                const type = (this.prng() > 0.7) ? 'stone' : level.name;
    ...
    _createRock(scene, type, position) {
        ...
        // Use the preset's fixed seed for ores, or a random one for stone
        const shapeSeed = (preset.seed !== null) ? preset.seed : this.prng() * 100;
        const noise = new Noise(shapeSeed);

        // --- Geometry Generation ---
        const geometry = new THREE.SphereGeometry(1, 32, 32); // Use fewer segments for better performance
        ...
        const uniforms = {
            ...
            // Pass lighting info from Game.js
            sunColor: { value: new THREE.Color(0xffffff) },
            hemiSkyColor: { value: new THREE.Color(0xffffff) },
            hemiGroundColor: { value: new THREE.Color(0xffffff) },
            lightDir: { value: new THREE.Vector3(0.5, 1, 0.5) },
            // Other material properties
            matRoughness: { value: preset.roughness },
            matMetalness: { value: preset.metalness },
            texFrequency: { value: 5.0 },
            bumpStrength: { value: 0.1 },
            texSeed: { value: this.prng() * 100 },
        };
        ...
        // Use the PRNG for scale and rotation
...
// (This includes the corrected, lighting-aware fragment shader)
...
const rockFragmentShader = `
    uniform vec3 color;
    uniform vec3 color2;
    uniform float texFrequency;
    uniform float bumpStrength;
    uniform float texSeed;
    
    // Updated uniforms to receive lighting from Game.js
    ...
    // FBM Noise function for procedural texturing
    ...
    void main() {
        ...
        // 3. Combine lights and apply to rock color
